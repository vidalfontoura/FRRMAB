...
public class AdaptiveIterationLimitedListBasedTA extends AcceptanceCriterion {
    ...
    /**
     * A counter indicating the number of worsening solutions accepted
     * consecutively *
     */
    ...
    /**
     * A threshold list to keep the best threshold list when the
     ...
     * @param	adaptationLimitMultiplier	a multiplier to decide about when to
    ...
    public AdaptiveIterationLimitedListBasedTA(int listLength, double initialFitness, int adaptationLimitMultiplier, long totalExecTime, Random r) {
        ...
        //System.out.println("START TIME ACCEPTANCE: " + startTime);
    ...
    public boolean accept(double newFitness, double currentFitness, double bestFitness) {
        ...
        if (limitCounterForThresholdChange >= iterationLimitForThresholdChange) {
            ...
            if (thresholdIndex <= bestFitnessList.size() - 1) {
                ...
                Vars.isAtStuck = true;
                //System.out.println("Busca esta parada.");
                ...
                if (!Vars.useOverallBestSln && !Vars.noMoreRestart) {
                    ...
                    //System.out.println("###########ENTROUAQUI: " + System.currentTimeMillis());
                   // System.out.println("TimeDiff: " + (System.currentTimeMillis() - startTime));
                    if (2 * (System.currentTimeMillis() - startTime) < totalExecTime) {
                        /* Active re-initialisation */
                        Vars.restartSearch = true;
                       // System.out.println("###########ENTROUAQUI1");
                        /*  */
    ...
    /**
     * Update the best threshold list if a new best solution is found during a
     * current re-initialisation
     *
     * @param bestFitnessList	the list of recently found best solutions
     * (threshold list)
     */
    ...
    /**
     * Set the threshold list with the values from the case where the best
     * solution found among all restarts
     */
    ...
    /**
     * Get the current threshold value for the adaptive iteration limited
     * list-based threshold accepting criterion
     */
